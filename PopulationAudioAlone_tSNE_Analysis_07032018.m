...
%%%%%%%%%%%%%%%%%%%%% Remove Noise and get the MFCC analysis data, including timestamps of each segment of the data that cross an arbitrary threshold %%%%    
   ...
   for FileNum=1:length(FileIndex)
      Temp_MFCCs=[]; 
   ...
   try
      ...
      errordlg('tNSE could not be complited, try reducing the threshold in order to include more data in the analysis. It might also be that your sampling rate in one of the files is not 250kHz (as should be for using the software)','analysis Error'); 
      return;
   end
   ...
   for GroupNum=1:length(FinalGroupsNamesForPresentation)
      tSNE_AnalysisData3D_Scatter3_Obj{GroupNum} = ...
         scatter3(PopAudioAlone_tSNE_AnalysisData3D(find(label==GroupNum), 1), ...
                  PopAudioAlone_tSNE_AnalysisData3D(find(label==GroupNum), 2), ...
                  PopAudioAlone_tSNE_AnalysisData3D(find(label==GroupNum), 3), ...
                  10,label(find(label==GroupNum)),...
                  'filled','DisplayName',char(FinalGroupsNamesForPresentation{GroupNum}));
      legend('show');
   end
   view(40,35)
   grid;
...
function [Temp_MFCCs,TempTimeFramesAfterNoiseRemover]=GetMFCC_from_data(AudioBehavioralpath, AudioBehavioralfile, NewFolderName,handles)
   ...
   try
      ...
      [TempNewSignal, TempTimeFramesAfterNoiseRemover]=noise_remover_15K_FINAL(TempAudioData,250000,handles); 
      disp(['Noise was removed and vocalizations were detected for file name: ' AudioBehavioralfile]) 
      [Temp_MFCCs, ~, ~ ] = MFCC_Gili_15K(TempNewSignal, 250000);
   catch
      Temp_MFCCs=[]; 
      TempTimeFramesAfterNoiseRemover=[];
   end
