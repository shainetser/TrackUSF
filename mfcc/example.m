...
%   using the MFCC routine.
    ...
    % Define variables
    Tw = 6;                % analysis frame duration (ms)
    Ts = 3;                % analysis frame shift (ms)
    ...
    [ MFCCs, FBEs, frames ] = mfcc( HET_fams, Fs, Tw, Ts, alpha, @hamming, [LF HF], M, C+1, L );
    ...
    xlabel( 'Time (s)' ); 
    ylabel( 'Amplitude' ); 
    title( 'Speech waveform - HET_fams'); 
