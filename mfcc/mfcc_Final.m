function [ CC, FBE, frames ] = mfcc_Final( speech, fs, Tw, Ts, alpha, window, R, M, N, L )
...
%           [ MFCCs, FBEs, frames ] = ...
%                           mfcc_Final( speech, fs, Tw, Ts, alpha, hamming, R, M, C, L );
%       
...
%   Author: Kamil Wojcicki, September 2011
    ...
    if( nargin~= 10 ), help mfcc_Final; return; end; 
    ...
    % Forward and backward mel frequency warping that bias the high frequencies and not the low ones.
    % Suggested by Gili and Shai 19-6-18  assuming min and max frequencies of 15000Hz and 100000Hz
%     hz2mel = @( hz )( 1127*log((1+100000)/700)+1127*log((1+15000)/700)-1127*log((1+hz)/700) );     % Hertz to mel warping function
%     mel2hz = @( mel )( exp(((1127*log((1+100000)/700)+1127*log((1+15000)/700))-mel)/1127)*700-1 ); % mel to Hertz warping function

 % Forward and backward Hz to mel as is.
